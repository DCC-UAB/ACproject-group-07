...
def train_model(df, feature_columns, target):
    ...
    #Evaluar amb MSE
    cv_mse_scores = cross_val_score(model, X, y, scoring='neg_mean_squared_error', cv=5)  # 5 folds de validació creuada

    # Convertir les puntuacions negatives en valors positius
    cv_mse_scores = -cv_mse_scores
    # Calcular el MSE mitjà i la desviació estàndard
    ...
    # 4. Obtenir l'OOB Score i calcular l'Error OOB
    oob_score = model.oob_score_  # Aquest és el coeficient R² basat en les mostres OOB
    oob_error = 1 - oob_score  # Error OOB com a complement del R²
    
    # Mostrar els resultats
    print(f"OOB R² Score: {oob_score:.4f}")
    print(f"OOB Error: {oob_error:.4f}")
    print(f"Cross-Validation MSE (mitjana): {mean_cv_mse:.4f}")
    print(f"Cross-Validation MSE (desviació estàndard): {std_cv_mse:.4f}")
