...
def train_with_cross_validation(features_df, n_folds=5, random_state=42):
    """
    Entrena i avalua el model Random Forest utilitzant k-fold cross validation.
    """
    # Preparar dades
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X), 1):
        ...
        # Configurar i entrenar Random Forest
    ...
    results_df.to_csv('results/rfg_fold_results_2.csv', index=False)
    
    # Crear i guardar plots
    fig = plot_fold_results(fold_results)
    fig.savefig('results/rfg_fold_results_2.png')
    plt.close()
